...
'''We see how our training error went up but our test error went down, which makes 
sense as our model is performing slightly better on test data at the cost of performing 
slightly worse on training data.'''

'''
We see how L2 regularization shrinks the coefficients as well, and while there are 
some coefficients that are very small, no coefficient is set to zero. The amount by 
which the feature coefficients get shrunk is related to the alpha we choose in implementing 
regularization. We’re now ready to learn the things to consider in choosing alpha.'''