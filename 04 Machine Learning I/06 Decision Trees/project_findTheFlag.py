...
print(f'Best accuracy {round(max_acc,3)*100}% at depth {best_depth}')

#Refit decision tree model with the highest accuracy and plot the decision tree
dTree = DecisionTreeClassifier(random_state = 10, max_depth=best_depth)
dTree.fit(train_data, train_labels)

tree.plot_tree(dTree, feature_names=train_data.columns, class_names=['Europe', 'Ocenia'], filled=True)
plt.show()
...
for i in ccp:
    dt_prune = DecisionTreeClassifier(random_state = 10, max_depth = best_depth, ccp_alpha=i)
...
print(f'Best accuracy {round(max_acc2,3)*100}% at ccp {best_ccp}')

#Fit a decision tree model with the values for max_depth and ccp_alpha found above
dt_prune_ccp = DecisionTreeClassifier(random_state=10, max_depth=best_depth, ccp_alpha=best_ccp)
dt_prune_ccp.fit(train_data, train_labels)


#Plot the final decision tree
tree.plot_tree(dt_prune_ccp, feature_names=train_data.columns, class_names=['Europe', 'Ocenia'], filled=True)
plt.show()